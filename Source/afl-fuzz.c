...
/* mrt 28 modified code to log requests and responses*/
int send_over_network()
{
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    u32 new_bytes = response_buf_size - prev_buf_size;

    fprintf(fp, "REQ: ");
    fwrite(kl_val(it)->mdata, 1, kl_val(it)->msize, fp);
    fprintf(fp, "RESP: ");

    if (new_bytes > 0) {
        unsigned int state_count = 0;
        unsigned int* states = extract_response_codes_ftp(response_buf + prev_buf_size, new_bytes, &state_count);

        if (state_count > 1) {
            fprintf(fp, "%u", states[state_count - 1]);
        } else {
            fprintf(fp, "NO_CODE");
        }
...
AFLNET_REGIONS_SELECTION:;
  ...
  doing_det = 1;
  goto havoc_stage; // This is to skip the simple bitflip and dictionary construction (as the commend below also says)
  /*********************************************
...
skip_bitflip:
  ...
  goto skip_arith; //Skipping the arithmetic inc/dec
...
skip_arith:
  ...
  for (i = len; i < len; i++) {// Skipp this loop
  ...
  goto skip_interest; //skip this section too
...
skip_interest:
  ...
  for (i = len; i <= len; i++) { // skip this loop
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch(21 + UR(5)){
        ...
        /*Replace with item from dict in sequence*/
        case 21: {
            u32 extra_len;
            u32 use_extra;
            u8* new_buf;
            ...
            do {
              use_extra = UR(extras_cnt);
              extra_len = extras[use_extra].len;
            } while (extra_len == temp_len && !memcmp(extras[use_extra].data, out_buf, extra_len));

            new_buf = ck_alloc_nozero(extra_len + 2);
            new_buf[extra_len] = 13;
            new_buf[extra_len + 1] = 10;
            memcpy(new_buf, extras[use_extra].data, extra_len);

            ck_free(out_buf);
            out_buf = new_buf;
            temp_len = extra_len + 2;
            break;
          }

        /*Insert new item from dict into sequence*/
        case 22: {
          u32 extra_len;
          u32 use_extra;
          u8* new_buf;
          ...
          new_buf = ck_alloc_nozero(temp_len + extra_len + 2);
          new_buf[temp_len + extra_len] = 13;
          new_buf[temp_len + extra_len + 1] = 10;

          memcpy(new_buf, out_buf, temp_len);
          memcpy(new_buf + temp_len, extras[use_extra].data, extra_len);

          ck_free(out_buf);
          out_buf = new_buf;
          temp_len = temp_len + extra_len + 2;
          break;
        }
        ...
        /* Delete a message from the selected M2 sequence */
        case 24: {

          u32 msg_count = M2_region_count;

          /* Need at least one message */
          if (msg_count <= 1) break;

          /* Select message inside M2 */
          u32 del_idx = UR(msg_count);

          /* Locate message boundaries */
          u32 cur_msg = 0;
          u32 msg_start = 0;
          u32 del_start = 0, del_end = 0;

          for (u32 i = 0; i < temp_len; i++) {

            /* Region/message boundary */
            if (i == temp_len - 1 ||
                (out_buf[i] == 13 && out_buf[i + 1] == 10)) {

              u32 msg_end =
                  (i == temp_len - 1) ? temp_len : i + 2;
          ...
          if (del_end <= del_start) break;

          u32 new_len = temp_len - (del_end - del_start);

          u8* new_buf = ck_alloc_nozero(new_len);

          memcpy(new_buf, out_buf, del_start);

          memcpy(new_buf + del_start,
                 out_buf + del_end,
                 temp_len - del_end);
        ...
        /* Swap two messages inside the selected M2 sequence */
        case 25: {
          ...
          u32 first_idx = UR(msg_count);
          u32 second_idx = UR(msg_count);

          while (second_idx == first_idx)
            second_idx = UR(msg_count);
          ...
          for (u32 i = 0; i < temp_len; i++) {
            ...
            if (i == temp_len - 1 ||
              ...
              starts[cur_msg] = msg_start;
              ends[cur_msg] = msg_end;

              msg_start = msg_end;
              cur_msg++;

              if (cur_msg >= msg_count) break;
            }
          }

          u32 first_start = starts[first_idx];
          u32 first_end   = ends[first_idx];

          u32 second_start = starts[second_idx];
          u32 second_end   = ends[second_idx];

          u32 first_len  = first_end - first_start;
          u32 second_len = second_end - second_start;
...
retry_splicing:
  ...
  if (use_splicing && splice_cycle++ < SPLICE_CYCLES &&
      queued_paths > 1 && M2_len > 1 && 0) {// no splice
